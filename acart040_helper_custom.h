...
enum colors{gray, yellow, green, darkGray} currColor;
...
typedef struct _letter{ // made to keep track of what's right and what's not, used in a 5 element array
  ...
  uint8_t letter;
  bool isRepeated = false; // checks if the inputed letter is repeated
} letterC;
...
bool validationProcess(char check[], const char* wordList[]){ // basically just binary search lol
  ...
  while (left <= right){
    ...
    found = strcmp(check, wordList[mid]);
...
void correctionChecks(char inputWord[], char correctWord[], letterC currCorrection[]){
  ...
  // I think its O(n^2) but we're only doing 25 runs and never goes above so I think its fine

  int used [] = {0, 0, 0, 0, 0}; // used for yellow checking, 1 if its yellow or green and we skip if that letter is not gray
  ...
  for (int i = 0; i < 5; ++i){
    if (currCorrection[i].currentColor == green){
      for (int j = 0; j < 5; ++j){
        if (inputWord[i] == correctWord[j] && !used[j]){
...
void turnOnLED(int LEDNum, letterC currColor[]){
  volatile uint8_t* port; // pointer to the port we want to change
  //unsigned char shift;

  // choosing the right port to use
  if (LEDNum < 2) {
    port = &PORTB;
  } 
  else if (LEDNum < 4) {
  ...
  if (currColor[LEDNum].currentColor == green){ // sets the color to green
    *port = SetBit(*port, 1, 1);
  }
  else{ // sets the color to yellow
...
void turnOffLEDS(){
  PORTB &= 0xC0;
  PORTC = 0x10; // keeps the buzzer value unchanged
  PORTD &= 0x03; // doesn't mess with the tx and rx pins; 
}
